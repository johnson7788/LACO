...
def safe_div(numerator, denominator):
    ...
    numerator, denominator = tf.cast(numerator, dtype=tf.float32), tf.cast(denominator, dtype=tf.float32)
...
def pr_re_fbeta(cm, pos_indices, beta=1):
    ...
    diag_sum = tf.reduce_sum(input_tensor=tf.linalg.tensor_diag_part(cm * cm_mask))

    cm_mask = np.ones([num_classes, num_classes])
    cm_mask[:, neg_indices] = 0
    tot_pred = tf.reduce_sum(input_tensor=cm * cm_mask)

    cm_mask = np.ones([num_classes, num_classes])
    cm_mask[neg_indices, :] = 0
    tot_gold = tf.reduce_sum(input_tensor=cm * cm_mask)
...
def metrics_from_confusion_matrix(cm, pos_indices=None, average='micro',
    ...
    if average == 'micro':
        ...
        for idx in pos_indices:
            ...
            cm_mask[idx, :] = 1
            n_golds.append(tf.cast(tf.reduce_sum(input_tensor=cm * cm_mask), dtype=tf.float32))

        if average == 'macro':
            pr = tf.reduce_mean(input_tensor=precisions)
            re = tf.reduce_mean(input_tensor=recalls)
            fbeta = tf.reduce_mean(input_tensor=fbetas)
            return pr, re, fbeta
        if average == 'weighted':
            n_gold = tf.reduce_sum(input_tensor=n_golds)
